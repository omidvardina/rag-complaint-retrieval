...
embedding_dir = drive / "consumer_complaints_recursive_bge_embeddings_v1_parts"

checkpoint_path = Path("recursive_pgvector_load_checkpoint.json")

embedding_files = sorted(
    embedding_dir.glob("recursive_chunks_part_*_embeddings.parquet")
)
...
for file_idx in range(start_file, len(embedding_files)):
    ...
    if (file_idx + 1) % 50 == 0 or file_idx == 0:
        ...
        cur.execute("SELECT COUNT(*) FROM recursive_chunks;")
